...
- (void)viewDidLoad
{
    ...
    self.navigationItem.prompt = [NSString stringWithFormat:@"Add %i photo%@ to \"%@\"", self.assetPickerState.selectedAssets.count, self.assetPickerState.selectedAssets.count > 1 ? @"s" : @"", self.assetPickerState.selectedFor];
...
{
    NSLog(@"Num Rows: %d", (self.fetchedAssets.count + self.assetsPerRow - 1) / self.assetsPerRow);
...
{    
    ...
    // Get the range of assets from fetchedAssets.
    NSArray *aFetchedAssets = [self.fetchedAssets objectsAtIndexes:indexSet];
    
    // Loop each image in ASAssets
    [aFetchedAssets enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        // asset prepare object
        WSAssetWrapper *fetchedAssetWrapper = obj;
        
        // Loop in selected Assets array
        [self.assetPickerState.selectedAssets enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            //prepare wrapper object
            ALAsset *selectedAsset = obj;
            if (fetchedAssetWrapper.asset) {
                if (selectedAsset) {
                    
                    // Compare and assign selected *BOOL
                    NSLog(@"%@ : %@", fetchedAssetWrapper.asset.defaultRepresentation.url, selectedAsset.defaultRepresentation.url);
                    if ([[fetchedAssetWrapper.asset.defaultRepresentation.url description] isEqualToString:[selectedAsset.defaultRepresentation.url description]] ) {
                        fetchedAssetWrapper.selected = YES;
                        // Stop loop
                        NSLog(@"Selected");
                        *stop = YES;
                    }else{
                        fetchedAssetWrapper.selected = NO;
                    }
                }
            }
        }];
    }];
    return aFetchedAssets;
}
