...
@synthesize selectedFor = _selectedFor;
...
- (id)initWithDelegate:(id <WSAssetPickerControllerDelegate>)delegate
{
    ...
    self.assetPickerState.selectedFor = self.selectedFor;
...
- (id)initWithDelegate:(id <WSAssetPickerControllerDelegate>)delegate withAlbumName:(NSString *)albumName
{
    self.selectedFor = albumName;
    // Create the Album TableView Controller.
    WSAlbumTableViewController *albumTableViewController = [[WSAlbumTableViewController alloc] initWithStyle:UITableViewStylePlain];
...
{    
    ...
    if ([STATE_KEY isEqualToString:keyPath]) {     
        ...
        NSLog(@"Total selected: %d", self.assetPickerState.selectedCount);
    }
}
